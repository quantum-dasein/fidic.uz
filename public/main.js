...
(function () {
  ...
  window.FidicReportExport = function (options) {
    ...
    var preparedBy = data.preparedBy || 'Bridge Consult / FIDIC.uz';
    var assumptions = Array.isArray(data.assumptions) ? data.assumptions : [];
    var limitationNote = data.limitationNote || '';
    var labels = {
      ru: {
    ...
    l.project = l.project || (lang === 'ru' ? 'Проект / пакет' : lang === 'uz' ? 'Loyiha / paket' : 'Project / package');
    l.preparedBy = l.preparedBy || (lang === 'ru' ? 'Подготовлено' : lang === 'uz' ? 'Tayyorladi' : 'Prepared by');
    l.generatedAt = l.generatedAt || (lang === 'ru' ? 'Время формирования' : lang === 'uz' ? 'Yaratilgan vaqt' : 'Generated timestamp');
    l.source = l.source || (lang === 'ru' ? 'Источник' : lang === 'uz' ? 'Manba' : 'Source');
    l.clientReady = l.clientReady || (lang === 'ru' ? 'Паспорт отчёта' : lang === 'uz' ? 'Report pasporti' : 'Report passport');
    l.assumptions = l.assumptions || 'Assumptions';
    l.limitation = l.limitation || (lang === 'ru' ? 'Ограничение использования' : lang === 'uz' ? 'Foydalanish chegarasi' : 'Limitation note');
    ...
    var projectPillHtml = projectName ? '<span class="meta-pill">' + esc(projectName) + '</span>' : '';
    var actionsHtml = list(inferredActions);
    var host = '';
    try { host = new URL(sourceUrl).host; } catch (_) { host = 'fidic.uz'; }
    var reportPassportRows = [
      [l.project, projectName || '-'],
      [l.preparedBy, preparedBy],
      [l.generatedAt, generatedAt.toLocaleString(lang)],
      [l.source, sourceUrl]
    ];
    var reportPassport = '<section class="report-section report-passport"><h2>' + esc(l.clientReady) + '</h2><table>' +
      reportPassportRows.map(function (row) {
        return '<tr><th>' + esc(row[0]) + '</th><td>' + esc(row[1]) + '</td></tr>';
      }).join('') +
      '</table></section>';
    var assumptionsHtml = assumptions.length
      ? '<section class="report-section"><h2>' + esc(l.assumptions) + '</h2>' + list(assumptions) + '</section>'
      : '';
    var limitationHtml = '<section class="report-section report-limitation"><h2>' + esc(l.limitation) + '</h2><p>' + esc(limitationNote || l.generated) + '</p></section>';
    ...
    var html = '<!doctype html><html lang="' + esc(lang) + '"><head><meta charset="utf-8">' +
      ...
      '<div class="cover-grid"><div><h1>' + esc(title) + '</h1><p class="summary">' + esc(inferredSummary) + '</p><p class="source">' + esc(subtitle) + '</p><div class="date">' + generatedAt.toLocaleString(lang) + '</div><div class="meta-row">' + statusHtml + projectPillHtml + '<span class="meta-pill">' + esc(host) + '</span><span class="meta-pill">' + esc(l.notAdvice) + '</span></div></div>' +
